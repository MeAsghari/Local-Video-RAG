# 🎬 Local Video RAG: Edge AI Search Engine
...
-   🚀 **High Performance:** Utilizes C++ for rapid frame extraction and preprocessing.
    
-   🧠 **Edge AI:** Runs state-of-the-art AI models (CLIP) on CPU using **Int8 Quantization** (reducing RAM usage by ~4x).
    
-   🔍 **Semantic Search:** Deep understanding of content (objects, actions, colors) using Vision-Language models.
    
-   📈 **Smart Smoothing:** Custom "Temporal Sliding Window" algorithm to increase detection accuracy and reduce noise.
    
-   🔒 **Privacy First:** All processing happens locally on your device; no data is sent to the cloud.
    
-   🎥 **Instant Playback:** Automatically generates an HTML player to jump to the exact timestamp of the result.
...
[![Watch the Demo](cover.png)](demo.mp4)

_(Click the image above to play the video)_
...
> 🔍 **Search:** "a girl reading a book"
> 
> 👉 **Result:** Found near 00:05 (Confidence: 75%)

## 📂 File Structure

    Edge-Video-Semantic-Search/
    ├── src/
    │   ├── main.cpp           # Main Indexer source code (C++)
    │   └── search_engine.py   # Search Interface (Python)
    ├── models/                # ONNX models location (generated by notebook)
    ├── build/                 # Compiled outputs (Exe, Binary Index)
    ├── necessary-files/       # Runtime requirements (DLLs, Test Video)
    ├── Local-Video-RAG-Report.docx  # Full technical report
    ├── Load-Model.ipynb       # Model download & quantization script
